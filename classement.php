...
$sql = "SELECT DISTINCT * FROM membres ORDER BY nbrVentes DESC LIMIT 30 ";
...
if (isset($_SESSION['id'])) {
  ...
  $rang = $bdd2->prepare("SELECT pseudo, test FROM (SELECT *, row_number() OVER (ORDER BY nbrventes DESC) test FROM membres) membres WHERE id = ?");
      ...
      <div class="l-wrapper">
        <div class="c-header"><img class="c-logo" src="images/foodtrocs0.1.png" draggable="false" style="height:100px; width:auto;" />
          <select class="c-button c-button--primary" onchange="location = this.value;">
            <option>Par nombre de ventes</option></a>
            <option value="classement_achats.php">Par nombre d'achats</option>
          </select>
        </div>
        <div class="l-grid">
              ...
              <div class="c-card__body">
                <div class="u-display--flex u-justify--space-between">
                  ...
                  <div class="u-text--right">
                    <div class="u-text--small , blanc">Mes ventes</div>
                    <h2 class="blanc"><?php echo $userinfo['nbrVentes'] ?> soit <?php echo $userinfo['ventes'] ?>€</h2>
                  </div>
                </div>
              </div>
            </div>
                ...
                <ul class="c-list" id="list">
                  <li class="c-list__item">
                    <div class="c-list__grid">
                        ...
                        ?>
                            <?php while ($row = mysqli_fetch_array($result)) { ?>
                              <div class="c-list__grid" style="margin-left:-110px">
                                <div id="<?php echo $rang?>" class="c-flag c-place u-bg--transparent"><?php echo "</br>". $rang++ ?></div>
                                <div class="c-media">
                                ...
                                <div class="u-text--right c-kudos">
                                  <div class="u-mt--8" style="margin-right:-100px"></br>
                                    <strong><?php echo $row['nbrVentes'] ?></strong>
                                  </div>
                                </div>
                              </div>
                            <?php } ?>
                      ...
                      <div class="u-text--right u-text--small u-text--medium">ventes</br></br>
                        <?php
                        if ($result = mysqli_query($bdd, $sql) and $result2 = mysqli_query($bdd, $sql2)) {
                          if (mysqli_num_rows($result) > 0) {
                        ?>
